 ...
 */
const API_BASE = "http://localhost:8080/hms/api";
const TOKEN_KEY = "hms_admin_token";
const ROLE_KEY = "hms_user_role";
const NAME_KEY = "hms_user_name";
// ----- tokens / storage -----
...
function clearUserName() { localStorage.removeItem(NAME_KEY); }
...
function setOut(id, html, show = true) {
    const el = document.getElementById(id);
    if (el) {
        el.innerHTML = html;
...
async function populatePatientDropdowns() {
    try {
        ...
        document.querySelectorAll(".select-patient-dropdown").forEach(sel => {
...
function applyAdminUi() {
    ...
    document.querySelectorAll(".admin-secured").forEach((panel) => {
        const lock = panel.querySelector(".lock-overlay");
        const content = panel.querySelector(".admin-content");
        if (lock)
            lock.style.display = admin ? "none" : "";
        if (content)
            content.style.display = admin ? "block" : "none";
    });
    // show admin-only sidebar nav buttons only for admins
    document.querySelectorAll(".nav-button.locked-nav").forEach(b => {
...
async function refreshDashboardStats() {
    ...
    try {
        if (isAdmin()) {
            ...
            if (rev)
                byId("stat-revenue").textContent = rev.value;
        }
    }
    catch (_d) { /* ignore */ }
}
...
const app = {
    ...
    switchTab(tab) {
        ...
        // update header title
        if (tab === "dashboard")
            byId("header-display-title").textContent = "Dashboard Overview";
        else {
    ...
    switchSubTab(parent, sub) {
        ...
        if (!p)
            return;
        p.querySelectorAll(".sub-panel").forEach(s => s.style.display = "none");
        const t = p.querySelector("#" + sub);
        if (t)
            t.style.display = "block";
        p.querySelectorAll(".sub-tab-btn").forEach(b => b.classList.remove("active"));
        const b = p.querySelector(`.sub-tab-btn[data-subtab="${sub}"]`);
        if (b)
    ...
    async discharge() {
        try {
            const r = await api("POST", "/discharges", {
    ...
    async recordPayroll() {
        ...
        try {
            const r = await api("POST", "/payroll", {
    ...
    async loadPayroll() {
        if (!isAuthed())
            return;
        try {
            const list = await api("GET", "/payroll", undefined);
    ...
    async loadConfig() {
        ...
        try {
            const list = await api("GET", "/config", undefined);
            const c = document.getElementById("cfg-list");
            if (!c)
                return;
            c.innerHTML = list.map(cfg => `<div class="cfg-row"><label>${cfg.key}</label>
                 <input id="cfg-${cfg.key}" value="${cfg.value}">
                 <button class="btn btn-secondary" onclick="app.saveConfig('${cfg.key}')">Save</button></div>`).join("");
    ...
    async saveConfig(key) {
        ...
        try {
            const r = await api("POST", "/config", { key, value: input.value });
    ...
    async loadReports() {
        ...
        try {
            const list = await api("GET", "/reports", undefined);
            const c = document.getElementById("reports-output-container");
            if (c)
                c.innerHTML = list.map(r => `<div class="report-item"><b>${r.title}:</b> ${r.detail}</div>`).join("");
...
function boot() {
    ...
    if (timeEl)
        timeEl.textContent = now.toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" });
    ...
    if (lbtn)
        lbtn.addEventListener("click", () => app.switchTab("login"));
    // sign out button in sidebar
    const sbtn = document.getElementById("sidebar-logout-btn");
    if (sbtn)
        sbtn.addEventListener("click", () => app.logoutAdmin());
    populatePatientDropdowns();
    refreshDashboardStats();
}
